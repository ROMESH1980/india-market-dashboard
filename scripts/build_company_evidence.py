...
# =========================================================
# STRICT AUTO VERIFICATION
# =========================================================

def normalized_source_date(value):
    dt = parse_date(value)
    if dt is None:
        return clean(value) or RUN_DATE
    return dt.date().isoformat()


def auto_verifiable_capex(item):
    ...
    try:
        strength = int(item.get("evidenceStrength", 0))
    except Exception:
        strength = 0
    ...
    if event_class not in AUTO_VERIFY_CAPEX_CLASSES:
        return False

    if strength < AUTO_VERIFY_CAPEX_MIN_STRENGTH:
        return False

    if not reason or not valid_source(source):
        return False

    source_lower = source.lower()

    # Auto verification only from NSE primary filing links.
    if (
        "nseindia.com" not in source_lower
        and
        "nsearchives.nseindia.com" not in source_lower
    ):
        return False
    ...
    if reject_text(combined):
        return False

    if contains_any(combined, CAPEX_NEGATIVE):
        return False
...
def promote_auto_verified_capex(
    evidence,
    evidence_stocks,
    symbol,
    item,
):
    # Existing manual/verified evidence always wins.
    if verified_exists(
        evidence_stocks,
        symbol,
        "capex",
    ...
    if not auto_verifiable_capex(item):
        return False

    stock_block = evidence_stocks.setdefault(
        symbol,
        {},
    )

    stock_block["capex"] = (
        verified_record_from_candidate(item)
    )

    evidence["stocks"] = evidence_stocks
    return True
...
def build():
    ...
    for index, stock in enumerate(
        ...
        # =================================================
        # STRICT AUTO-VERIFY: CAPEX ONLY
        # =================================================

        if (
            not capex_already_verified
            and capex_items
        ):
            best_capex = capex_items[0]

            if promote_auto_verified_capex(
                evidence,
                evidence_stocks,
                symbol,
                best_capex,
            ):
                auto_verified_capex += 1
                best_capex["candidateStatus"] = (
                    "AUTO_VERIFIED"
                )
    ...
    # =====================================================
    # SAVE AUTO-VERIFIED EVIDENCE
    # =====================================================

    evidence.setdefault("_meta", {})
    evidence["_meta"]["updated"] = RUN_DATE
    evidence["_meta"]["autoVerification"] = (
        "Strict NSE primary-filing CAPEX auto-verification enabled."
    )
    ...
    result = {
        "_meta": {
            ...
            "important": (
                "Strong NSE CAPEX evidence may be "
                "AUTO_VERIFIED. Ambiguous CAPEX "
                "and all Tailwind evidence remain "
                "REVIEW_REQUIRED."
            ),
