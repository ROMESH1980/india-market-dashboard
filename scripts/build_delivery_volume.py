import json
from pathlib import Path
from datetime import datetime, timezone, timedelta

import requests


ROOT = Path(__file__).resolve().parents[1]
DATA = ROOT / "data"

HEADERS = {
    "User-Agent": "Mozilla/5.0 (compatible; NSEMarketDashboard/1.0)",
    "Accept": "text/plain,text/csv,*/*",
}

# =========================================================
# SETTINGS
# =========================================================

# We need enough historical NSE sessions so that
# illiquid stocks can still find their previous
# 5 AVAILABLE delivery records.
HISTORY_SESSION_COUNT = 20

# Search calendar range.
# 45 days is enough for weekends, holidays and
# moderately illiquid stocks.
MAX_CALENDAR_LOOKBACK = 45

# Number of previous AVAILABLE stock records
# required for the average.
REQUIRED_PREVIOUS_RECORDS = 5


# =========================================================
# JSON HELPER
# =========================================================

def load_json(path, default):
    try:
        return json.loads(path.read_text())
    except Exception:
        return default


# =========================================================
# SAFE NUMBER
# =========================================================

def to_float(value):
    try:
        if value is None:
            return None

        value = str(value).strip()

        if value == "":
            return None

        return float(value)

    except Exception:
        return None


# =========================================================
# FETCH NSE MTO FILE
# =========================================================

def fetch_mto_for_date(d):

    ddmmyyyy = d.strftime("%d%m%Y")

    url = (
        "https://nsearchives.nseindia.com/"
        f"archives/equities/mto/MTO_{ddmmyyyy}.DAT"
    )

    r = requests.get(
        url,
        headers=HEADERS,
        timeout=45,
    )

    r.raise_for_status()

    delivery = {}

    for line in r.text.splitlines():

        line = line.strip()

        if not line.startswith("20,"):
            continue

        parts = [
            p.strip()
            for p in line.split(",")
        ]

        # NSE MTO structure:
        #
        # 20,
        # Sr No,
        # Symbol,
        # Series,
        # Quantity Traded,
        # Deliverable Quantity,
        # Delivery %

        if len(parts) < 7:
            continue

        symbol = parts[2]
        series = parts[3]

        traded_qty = to_float(
            parts[4]
        )

        deliverable_qty = to_float(
            parts[5]
        )

        delivery_pct = to_float(
            parts[6]
        )

        if not symbol:
            continue

        delivery[
            (
                symbol,
                series,
            )
        ] = {
            "tradedQty":
                traded_qty,

            "deliverableQty":
                deliverable_qty,

            "deliveryPct":
                delivery_pct,
        }

    if not delivery:
        raise ValueError(
            f"No delivery rows found for {d}"
        )

    return delivery


# =========================================================
# LOAD HISTORICAL NSE SESSIONS
# =========================================================

def get_last_trading_sessions(
    required=HISTORY_SESSION_COUNT
):

    today = (
        datetime.now(timezone.utc)
        .date()
    )

    sessions = []

    for back in range(
        0,
        MAX_CALENDAR_LOOKBACK
    ):

        d = today - timedelta(
            days=back
        )

        # Skip Saturday / Sunday
        if d.weekday() >= 5:
            continue

        try:

            data = fetch_mto_for_date(
                d
            )

            sessions.append({
                "date": d,
                "data": data,
            })

            print(
                f"Delivery session loaded: "
                f"{d} ({len(data)} rows)"
            )

            if len(sessions) >= required:
                break

        except Exception as e:

            print(
                f"Skipping {d}: {e}"
            )

    if not sessions:

        raise RuntimeError(
            "No valid NSE delivery sessions found"
        )

    print(
        f"Total historical NSE delivery "
        f"sessions loaded: {len(sessions)}"
    )

    return sessions


# =========================================================
# FIND SYMBOL RECORD
# =========================================================

def find_delivery_record(
    session_data,
    symbol,
    series,
):

    # First try exact symbol + series

    record = session_data.get(
        (
            symbol,
            series,
        )
    )

    # Main-board fallback

    if record is None:

        record = session_data.get(
            (
                symbol,
                "EQ",
            )
        )

    return record


# =========================================================
# GET PREVIOUS 5 AVAILABLE STOCK RECORDS
# =========================================================

def get_previous_available_delivery_records(
    sessions,
    symbol,
    series,
):

    values = []
    dates = []

    # sessions[0] = TODAY
    #
    # Therefore start from sessions[1].
    #
    # We do NOT require the stock to appear
    # on every global NSE trading date.
    #
    # Missing stock-specific dates are skipped.
    #
    # We continue backwards until we collect
    # exactly 5 AVAILABLE delivery records.

    for session in sessions[1:]:

        record = find_delivery_record(
            session["data"],
            symbol,
            series,
        )

        if record is None:
            continue

        value = to_float(
            record.get(
                "deliverableQty"
            )
        )

        if value is None:
            continue

        values.append(
            value
        )

        dates.append(
            session["date"].isoformat()
        )

        if len(values) >= REQUIRED_PREVIOUS_RECORDS:
            break

    return values, dates


# =========================================================
# MAIN
# =========================================================

def main():

    stocks_path = (
        DATA /
        "stocks.json"
    )

    meta_path = (
        DATA /
        "meta.json"
    )

    stocks = load_json(
        stocks_path,
        [],
    )

    # =====================================================
    # LOAD NSE DELIVERY HISTORY
    # =====================================================

    sessions = (
        get_last_trading_sessions(
            required=HISTORY_SESSION_COUNT
        )
    )

    latest_session = (
        sessions[0]
    )

    today_date = (
        latest_session["date"]
        .isoformat()
    )

    print(
        f"Latest NSE delivery date: "
        f"{today_date}"
    )


    # =====================================================
    # COUNTERS
    # =====================================================

    updated_today_volume = 0
    updated_delivery_volume = 0
    updated_ratio = 0
    updated_delivery_pct = 0

    full_5day_average_count = 0
    incomplete_5day_count = 0


    # =====================================================
    # PROCESS STOCKS
    # =====================================================

    for row in stocks:

        symbol = row.get(
            "symbol"
        )

        series = (
            row.get("series")
            or ""
        )

        if not symbol:
            continue


        # =================================================
        # TODAY'S RECORD
        # =================================================

        today_record = (
            find_delivery_record(
                latest_session["data"],
                symbol,
                series,
            )
        )

        today_volume = None
        today_delivery = None
        official_delivery_pct = None


        if today_record:

            today_volume = (
                to_float(
                    today_record.get(
                        "tradedQty"
                    )
                )
            )

            today_delivery = (
                to_float(
                    today_record.get(
                        "deliverableQty"
                    )
                )
            )

            official_delivery_pct = (
                to_float(
                    today_record.get(
                        "deliveryPct"
                    )
                )
            )


        # =================================================
        # PREVIOUS 5 AVAILABLE STOCK DELIVERY RECORDS
        # =================================================

        previous_values, previous_dates = (
            get_previous_available_delivery_records(
                sessions,
                symbol,
                series,
            )
        )


        # =================================================
        # STRICT 5 AVAILABLE RECORD RULE
        # =================================================

        avg_5d = None

        if (
            len(previous_values)
            ==
            REQUIRED_PREVIOUS_RECORDS
        ):

            avg_5d = (
                sum(previous_values)
                /
                REQUIRED_PREVIOUS_RECORDS
            )

            full_5day_average_count += 1

        else:

            incomplete_5day_count += 1


        # =================================================
        # DELIVERY TIMES
        # =================================================

        ratio = None

        if (
            today_delivery is not None
            and
            avg_5d is not None
            and
            avg_5d > 0
        ):

            ratio = (
                today_delivery
                /
                avg_5d
            )


        # =================================================
        # DELIVERY %
        # =================================================

        delivery_pct = None

        # Prefer official NSE MTO delivery %

        if official_delivery_pct is not None:

            delivery_pct = (
                official_delivery_pct
            )

        # Safety fallback

        elif (
            today_delivery is not None
            and
            today_volume is not None
            and
            today_volume > 0
        ):

            delivery_pct = (
                today_delivery
                /
                today_volume
                *
                100
            )


        # =================================================
        # SAVE TODAY VOLUME
        # =================================================

        row["todayVolume"] = (

            round(today_volume)

            if today_volume is not None

            else None
        )


        # Keep old frontend field synchronized

        row["volume"] = (

            round(today_volume)

            if today_volume is not None

            else None
        )


        row["mtoTradedVolume"] = (

            round(today_volume)

            if today_volume is not None

            else None
        )


        # =================================================
        # SAVE DELIVERY DATA
        # =================================================

        row["todayDeliveryVolume"] = (

            round(today_delivery)

            if today_delivery is not None

            else None
        )


        row["avg5DayDeliveryVolume"] = (

            round(
                avg_5d,
                2,
            )

            if avg_5d is not None

            else None
        )


        row["deliveryVolumeRatio"] = (

            round(
                ratio,
                2,
            )

            if ratio is not None

            else None
        )


        row["deliveryPct"] = (

            round(
                delivery_pct,
                2,
            )

            if delivery_pct is not None

            else None
        )


        row["deliveryDate"] = (
            today_date
        )


        # =================================================
        # AUDIT FIELDS
        # =================================================
        #
        # Helpful if we ever need to verify
        # a stock's 5D calculation again.
        #

        row["deliveryHistoryCount"] = (
            len(previous_values)
        )

        row["deliveryHistoryDates"] = (
            previous_dates
        )

        row["deliveryHistoryValues"] = [

            round(x)

            for x in previous_values
        ]


        # =================================================
        # COUNTS
        # =================================================

        if today_volume is not None:

            updated_today_volume += 1


        if today_delivery is not None:

            updated_delivery_volume += 1


        if ratio is not None:

            updated_ratio += 1


        if delivery_pct is not None:

            updated_delivery_pct += 1


    # =====================================================
    # WRITE STOCKS.JSON
    # =====================================================

    stocks_path.write_text(

        json.dumps(
            stocks,
            ensure_ascii=False,
            separators=(",", ":"),
        )
    )


    # =====================================================
    # UPDATE META.JSON
    # =====================================================

    meta = load_json(
        meta_path,
        {},
    )


    meta["deliveryDate"] = (
        today_date
    )


    meta["deliverySessionsUsed"] = [

        x["date"].isoformat()
        for x in sessions
    ]


    meta["todayVolumeCount"] = (
        updated_today_volume
    )


    meta["deliveryVolumeCount"] = (
        updated_delivery_volume
    )


    meta["deliveryRatioCount"] = (
        updated_ratio
    )


    meta["deliveryPctCount"] = (
        updated_delivery_pct
    )


    meta["full5DayDeliveryAverageCount"] = (
        full_5day_average_count
    )


    meta["incomplete5DayDeliveryCount"] = (
        incomplete_5day_count
    )


    meta["deliveryAverageRule"] = (
        "Previous 5 available stock delivery records"
    )


    meta["deliveryAverageRequiredRecords"] = (
        REQUIRED_PREVIOUS_RECORDS
    )


    meta["deliveryHistorySessionsLoaded"] = (
        len(sessions)
    )


    meta["volumeSource"] = (
        "Official NSE MTO Quantity Traded"
    )


    # =====================================================
    # WRITE META.JSON
    # =====================================================

    meta_path.write_text(

        json.dumps(
            meta,
            indent=2,
        )
    )


    # =====================================================
    # FINAL LOG
    # =====================================================

    print({
        "todayTradingDate":
            today_date,

        "historicalSessionsLoaded":
            len(sessions),

        "todayVolumesCalculated":
            updated_today_volume,

        "deliveryVolumesCalculated":
            updated_delivery_volume,

        "full5DayAveragesCalculated":
            full_5day_average_count,

        "incomplete5DayHistories":
            incomplete_5day_count,

        "deliveryRatiosCalculated":
            updated_ratio,

        "deliveryPctCalculated":
            updated_delivery_pct,

        "deliveryAverageRule":
            "PREVIOUS_5_AVAILABLE_STOCK_RECORDS",

        "volumeSource":
            "NSE MTO Quantity Traded",
    })


if __name__ == "__main__":
    main()
